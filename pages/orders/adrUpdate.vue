	...
	<view class="content">
		<u-form class="u-m-l-60 u-m-r-80" :error-type="errorType" :model="addressForm" ref="addressFormRef">
			<u-form-item label-width="150" label="联系姓名" prop="name" required>
				<u-input v-model="addressForm.name" />
			</u-form-item>
			<u-form-item label-width="150" label="联系电话" prop="phone" required>
				<u-input v-model="addressForm.phone" />
			</u-form-item>
			...
			<u-form-item label-width="150" label="详细地址" prop="address" required>
				<u-input v-model="addressForm.address"/>
			</u-form-item>
		</u-form>
		<u-button type="primary" class="u-m-l-60 u-m-r-60" @click="submit">修改地址</u-button>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			async getAddressDetails(){
				const res = await this.$u.api.addressDetails(this.addressId);
			...
			submit(){
				this.$refs.addressFormRef.validate(async valid => {
					...
					await this.$u.api.addressUpdate(this.addressId,this.addressForm);
					this.getAddressDetails();
					this.$u.toast('修改地址成功');
				})
			},
